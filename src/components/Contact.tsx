    ...
    return (
        ...
        >
            <MaxWidthWrapper className="flex flex-col md:flex-row items-start md:items-stretch md:justify-between">
                <div className="w-full md:w-[33%] flex justify-start items-start h-auto flex-col md:pr-4 lg:pr-0">
                    <p
                        className={`tracking-[0.275em] text-(--text-color-3) font-family-2 text-sm reveal d1
                        ${visible ? "is-visible" : ""}`}
                    >
                        05 - CONNECT
                    </p>
                    <p
                        className={`font-family-1 text-2xl md:text-3xl lg:text-4xl text-(--text-color-1) mt-3 md:mt-6 font-semibold reveal d1
                        ${visible ? "is-visible" : ""}`}
                    >
                        Let's Work Together
                    </p>
                    <p
                        className={`font-family-2 md:text-sm lg:text-base text-(--text-color-3) mt-4 reveal d1
                        ${visible ? "is-visible" : ""}`}
                    >
                        Ready to collaborate on challenging projects or discuss opportunities in game development, systems programming, or full-stack engineering.
                    </p>

                    <div
                        className={`w-full bg-(--background-color-4) h-auto mt-8 lg:mt-12 px-4 lg:px-8 py-2.5 lg:py-4 border border-[color-mix(in_srgb,var(--background-color-3)_25%,transparent)] flex flex-col items-start justify-center font-family-2 gap-y-1 text-sm lg:text-base reveal d2
                        ${visible ? "is-visible" : ""}`}
                    >
                        <p className="text-(--text-color-1)">
                            Current Status
                        </p>
                    ...
                    {/* Social Media */}
                    <div
                        className={`hidden md:flex flex-col justify-center items-start mt-12 lg:mt-20 gap-y-4 font-medium text-xs lg:text-base reveal d3
                        ${visible ? "is-visible" : ""}`}
                    >
                        {/* Email */}
                        <a
                            className="nav-item flex items-center gap-x-3 cursor-pointer transition-colors duration-300"
                            style={{
                        ...
                        >
                            ...
                            <p className="nav-link">febriandika4567@gmail.com</p>
                        </a>
                        ...
                        >
                            ...
                            <p className="nav-link">github.com/theprodigy18</p>
                        </a>
                        ...
                        >
                            ...
                            <p className="nav-link">childprodigy18.itch.io</p>
                        </a>
                    </div>

                    {/* Location */}
                    <span
                        className={`hidden md:flex items-center gap-x-3 text-(--text-color-3) mt-8 lg:mt-12 text-xs lg:text-base reveal d3
                        ${visible ? "is-visible" : ""}`}
                    >
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                            className="size-5 lg:size-6">
                ...
                <div className="w-full md:w-[64%] flex flex-col items-start justify-center mt-8 md:mt-0">
                    <div className="flex flex-col w-full h-full justify-start items-start font-family-2">
                        <div
                            className={`flex justify-between items-center w-full reveal d2
                            ${visible ? "is-visible" : ""}`}
                        >
                            <div className="w-[48%]">
                                <label className="text-(--text-color-3) text-xs lg:text-sm font-medium">NAME</label>
                                <input
                                    type="text"
                                    name="name"
                                    value={formData.name}
                                    ...
                                    className="bg-(--background-color-4) w-full px-4 py-2.5 mt-2 border border-[color-mix(in_srgb,var(--background-color-3)_20%,transparent)] placeholder:text-[color-mix(in_srgb,var(--background-color-3)_40%,transparent)] text-xs lg:text-sm text-(--text-color-1) focus:outline-none focus:border-(--background-color-2) disabled:opacity-50"
                                />
                            </div>
                            <div className="w-[48%]">
                                <label className="text-(--text-color-3) text-xs lg:text-sm font-medium">EMAIL</label>
                                <input
                                    type="email"
                                    name="email"
                        ...
                        <div
                            className={`flex flex-col w-full mt-6 lg:mt-8 reveal d3
                            ${visible ? "is-visible" : ""}`}
                        >
                            <label className="text-(--text-color-3) text-xs lg:text-sm font-medium">SUBJECT</label>
                            <input
                                type="text"
                                name="subject"
                        ...
                        <div
                            ...
                            ${visible ? "is-visible" : ""}`}
                        >
                            <label className="text-(--text-color-3) text-xs lg:text-sm font-medium">MESSAGE</label>
                            <textarea
                                rows={5}
                                name="message"
                        ...
                        {/* Status Message */}
                        {status.message && (
                            <div className={`w-full mt-4 p-4 rounded ${status.type === 'success'
                        ...
                        <button
                            ...
                            disabled={isSubmitting}
                            className={`text-(--text-color-1) flex items-center justify-center gap-2 mt-6 lg:mt-8 tracking-wider px-6 lg:px-8 py-2.5 lg:py-3.5 bg-(--background-color-2) font-medium cursor-pointer transition-all duration-300 ease-in hover:bg-[color-mix(in_srgb,var(--background-color-2)_50%,transparent)] text-sm lg:text-base reveal d4
                            ${visible ? "is-visible" : ""} disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:bg-(--background-color-2)`}
                        >
                            {isSubmitting ? (
                                <>
                                    <svg
                ...
                {/* Social Media */}
                <div
                    className={`flex md:hidden flex-col justify-center items-start mt-12 lg:mt-20 gap-y-4 font-medium text-xs lg:text-base reveal d5
                    ${visible ? "is-visible" : ""}`}
                >
                    {/* Email */}
                    <a
                ...
                {/* Location */}
                <span
                    className={`flex md:hidden items-center gap-x-3 text-(--text-color-3) mt-8 lg:mt-12 text-xs lg:text-base reveal d5
                    ${visible ? "is-visible" : ""}`}
                >
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                        className="size-5 lg:size-6">
